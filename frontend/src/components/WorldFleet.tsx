...
const TICK = 120;
const TIME_SCALE = 26;
const CAP = 160; // max markers rendered at once (keeps DOM/RAM flat at any zoom)
...
function icon(v: WorldVehicle): L.DivIcon {
  if (domainOf(v.car) === "road") {
    return L.divIcon({
      ...
      iconSize: [16, 16],
      iconAnchor: [8, 8],
      html: `<div style="transform:rotate(${v.heading}deg);filter:drop-shadow(0 0 4px #00E5FF)">
        <svg width="16" height="16" viewBox="0 0 16 16"><path d="M8 1 L13 14 L8 11 L3 14 Z"
        fill="#00E5FF" stroke="#0A0B0F" stroke-width="0.6"/></svg></div>`,
    });
  }
  return L.divIcon({
    className: "",
    iconSize: [22, 22],
    iconAnchor: [11, 11],
    html: `<div style="font-size:16px;line-height:22px;text-align:center;filter:drop-shadow(0 0 5px #00E5FF)">${iconOf(v.car)}</div>`,
  });
}
  ...
  useEffect(() => {
    api.current = {
      add: (car) => {
        ...
        fleet.current.push(spawnAt([c.lat, c.lng], car, car?.domain ?? "road"));
        cull(false);
      },
    };
        ...
        <Marker key={v.id} position={v.pos} icon={icon(v)}>
          <Popup>
            <div className="text-xs">
              ...
              <br />
              {flag(v.car.country)} {v.car.country} · {v.car.type} · {domainOf(v.car)}
            </div>
          </Popup>
        </Marker>
      ))}
